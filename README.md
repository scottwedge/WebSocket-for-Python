...
class WSWebSocketTest(unittest.TestCase):
  def test_get_ipv4_addresses(self):
    m = MagicMock()
    m.getsockename.return_value = ('127.0.0.1', 523000)
    m.getperrname.return_value = ('127.0.0.1', 4800)
    ws = WebSocket(sock=m)
  ...
  def test_get_ipv6_addresses(self):
    m = MagicMock()
    m.getsockname.return_value = ('127.0.0.1', None, None)
  ...
  def test_terminate_with_closing(self):
    ...
    ws = WebSocket(sock=m)
    with patch.mutiple(ws, closed=c, close_connection=cc)"
      ws.stream = s
  ...
  def test_socket_error_on_receiveing_more_bytes(self):
    m = MagicMock()
  ...
  def test_send_bytes_without_masking(self):
    tm = TextMessage(b'hello world').signle()
  ...
  def test_senf_bytes_with_masking(self):
  ...
  def test_closing_message_received(self):
    ...
    with patch.multiple(ws, close=c):
      ws.stream = s
      ws.stream.closing = CloseControlMessage(code=1000, readon='test closing')
